 ...
 *
 * Embedding Strategy:
 * - Uses Title + Committees + Summary (when available) for semantic representation
 * - OpenAI text-embedding-3-small model (1536 dimensions)
 * - Batch request to OpenAI API (same order in = same order out)
 * - HTML tags cleaned from summary before embedding and DB storage
 *
...
function createEmbeddingText(title: string, committees: string, summary?: string | null): string {
  ...
  if (summary) {
    // Summary is already cleaned when passed in, but double-check
    const cleanSummary = summary.includes('<') ? cleanHtml(summary) : summary;
