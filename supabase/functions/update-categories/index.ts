...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const hallucinationCount = failed.filter(f => f.error && f.error.includes("HALLUCINATION")).length;
    const invalidCategoryCount = failed.filter(f => f.error && f.error.includes("Invalid category")).length;
    const dbErrorCount = failed.filter(f => f.error && f.error.includes("DB error")).length;
